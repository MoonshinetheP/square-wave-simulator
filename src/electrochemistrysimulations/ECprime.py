...
class ECprime:
    ...
    def __init__(self, input, E0, k0, a, k1, cR, cO, DR, DO, r, expansion, Nernstian, BV, MH):
        '''Waveform variables'''
        self.input = input
        ...
        for ix in range(1, self.n - 1):
            ...
            self.beta_R[ix] *= -2 * self.dR * (self.xminus + self.xplus) / self.denominator + self.K1
            self.gamma_R[ix] *= 2 * self.dR * self.xminus / self.denominator
            
            self.alpha_O[ix - 1] *= 2 * self.dO * self.xplus / self.denominator
            self.beta_O[ix] *= -2 * self.dO * (self.xminus + self.xplus) / self.denominator - self.K1
...
if __name__ == '__main__':
    ...
    shape = wf.CV(Eini = 0.0, Eupp = 0.5, Elow = 0.0, dE = 0.002, sr = 0.01, ns = 1)
    #shape = wf.DPV(Eini = 0, Efin = 0.5, dEs = 0.005, dEp = 0.01, pt = 0.01, rt = 0.03, st = 0.000001, detailed = False)
    instance = ECprime(input = shape, E0 = 0.25, k0 = 0.1, a = 0.5, k1 = 0.01, cR = 0.005, cO = 0.000, DR = 5E-6, DO = 5E-6, r = 0.15, expansion = 1.05, Nernstian = False, BV = True, MH = False)
