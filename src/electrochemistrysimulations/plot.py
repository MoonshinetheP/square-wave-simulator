...
'''
    ...
    def __init__(self, shape, data, display = True, animate = True, save = True):

        '''PARAMETER INITIALISATION'''
        self.shape = shape      # potential waveform object generated by waveforms.py
        self.data = data        # analysed oscilloscope data object generated using operations.py
        ...
        right, = ax2.plot(self.data.E[1:], self.data.i, linewidth = 1, linestyle = '-', color = 'red', marker = None, label = None, visible = True)     # plots the oscilloscope data from operations.py on the right-hand subplot
        ...
        ax2.set_xlim(np.amin(self.data.E) - (0.1 * (np.amax(self.data.E) - np.amin(self.data.E))), np.amax(self.data.E) + (0.1 * (np.amax(self.data.E) - np.amin(self.data.E))))        # sets the x-axis of the right-hand subplot to +/- 10% of the oscilloscope data's potential range
        ax2.set_ylim(np.nanmin(self.data.i) - (0.1 * (np.nanmax(self.data.i) - np.nanmin(self.data.i))), np.nanmax(self.data.i) + (0.1 * (np.nanmax(self.data.i) - np.nanmin(self.data.i))))        # sets the y-axis of the right-hand subplot to +/- 10% of the oscilloscope data's current range
