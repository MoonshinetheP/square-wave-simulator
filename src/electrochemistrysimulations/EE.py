...
class EE:
    ...
    def __init__(self, input, E0, k0, a0, E1, k1, a1, cR, cO, cY, DR, DO, DY, r, expansion, Nernstian, BV, MH):
        '''Waveform variables'''
        self.input = input
        ...
        for k in range(1,self.m):
            ...
            if self.Nernstian == True:
                ...
                self.C_O[0, k] = (self.C_R[1, k - 1] + (self.dR/self.dO) * self.C_O[1, k - 1])/((self.dR/self.dO) + np.exp(-self.theta[k-1])) - (self.C_O[1, k - 1] + (self.dO/self.dY) * self.C_Y[1, k - 1])/(1 + (self.dO/self.dY) * np.exp(self.theta2[k-1]))
            
                self.C_Y[0, k] = (self.C_O[1, k - 1] + (self.dO/self.dY) * self.C_Y[1, k - 1])/((self.dO/self.dY) + np.exp(-self.theta2[k-1]))
            ...
            self.flux = np.append(self.flux, (self.F * np.pi * self.r * self.cR * self.DR) * ((self.C_R[1, k] - self.C_R[0, k]) / (self.x[1] - self.x[0])) + (self.F * np.pi * self.r * self.cY * self.DY) * ((self.C_Y[1, k] - self.C_Y[0, k]) / (self.x[1] - self.x[0])))
...
if __name__ == '__main__':
    ...
    instance = EE(input = shape, E0 = 0.25, k0 = 0.1, a0 = 0.5, E1 = 0.75, k1 = 0.1, a1 = 0.5, cR = 0.005, cO = 0.000, cY = 0.000, DR = 5E-6, DO = 5E-6, DY = 5E-6, r = 0.15, expansion = 1.05, Nernstian = True, BV = False, MH = False)
